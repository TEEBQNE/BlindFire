...
public class P2HealthScript : MonoBehaviour {
	...
	void Update () {
		if (!setHealth) {
			setHealth = true;
			startHealth = objectToFollow.gameObject.GetComponent<MovementTest> ().health;
			currentHealth = startHealth;
		}

		currentHealth = objectToFollow.gameObject.GetComponent<MovementTest> ().health;
		/*if (currentHealth <= 0) {
			Destroy (gameObject);
		}*/
		if (currentHealth <= 0) {
