...
public class bossHealthBar : MonoBehaviour {
	...
	void Update () {
		if (!setHealth) {
			setHealth = true;
			startHealth = objectToFollow.gameObject.GetComponent<bossHealthScript> ().health;
			currentHealth = startHealth;
		}

		currentHealth = objectToFollow.gameObject.GetComponent<bossHealthScript> ().health;
		if (currentHealth <= 0) {
			Destroy (gameObject);
		}
	...
	void LateUpdate()
	{
		Vector3 localOffset = new Vector3 (xOffset, offset, 0f);
		// Translate our anchored position into world space.
		Vector3 worldPoint = objectToFollow.transform.Find("healthBarLoc").TransformPoint(localOffset);
